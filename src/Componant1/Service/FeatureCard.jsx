...
const FeatureCard = ({
  ...
  return (
    <div className="text-center shadow-shade overflow-hidden group px-10 pt-[60px] pb-8 rounded-lg bg-white relative z-10 before:absolute before:bottom-0 before:left-0 before:w-full before:h-0 before:bg-[url('/images/feature-box-shape.png')] before:bg-cover before:bg-center before:bg-no-repeat before:transition-all before:duration-500 before:-z-10 hover:before:h-full hover:before:top-0">
      ...
      <h4 className="font-FiraSans font-bold text-2xl sm:text-[26px] lg:text-2xl xl:text-[26px] text-HeadingColor-0 transition-all duration-500 group-hover:text-white mt-11 mb-2">
        {featureTitle}
      </h4>
      <p className="font-FiraSans text-TextColor2-0 transition-all duration-500 group-hover:text-white mb-[18px]">
        {featureDesc}
      </p>
      <Link
        to={featureUrl}
      ...
      >
        <button className="font-FiraSans font-medium text-white opacity-0 transition-all duration-500 group-hover:opacity-100">
          {featureBtn}
        </button>
      </Link>
    </div>
